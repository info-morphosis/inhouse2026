...
export default function RegistroMediosPage() {
  ...
  if (CERRADO) return <RegistroCerrado etiqueta="MEDIOS DE COMUNICACIÓN" titulo="Acreditación de Prensa" />
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await fetch('/api/registro', {
        ...
        body: JSON.stringify({ tipo: 'campana', slug: 'medios', ...form, whatsapp: normalizarWhatsappEc(form.telefono), empresa: form.empresa }),
      })
  ...
  return (
    ...
    <main className="max-w-lg mx-auto px-4 py-8">
      ...
      <div className="text-center mb-8">
        ...
        <h1 className="text-2xl font-bold">Acreditación de Prensa</h1>
      ...
      <form onSubmit={handleSubmit} className="card space-y-4">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          ...
          <div><label className="label">Medio / Empresa *</label>
            <input className="input-field" placeholder="Nombre del medio o empresa" required value={form.empresa} onChange={e => setForm(f => ({ ...f, empresa: e.target.value }))} /></div>
        </div>
        {error && <p className="text-red-400 text-sm">{error}</p>}
        <button type="submit" disabled={loading} className="btn-neon w-full text-center disabled:opacity-50">
          {loading ? 'Registrando…' : 'Confirmar acreditación'}
        </button>
      </form>
    </main>
    <EventFooter />
